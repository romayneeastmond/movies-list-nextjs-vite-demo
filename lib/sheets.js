...
function rowToMovie(row) {
  ...
  return {
    ...
    CurrentSeason:  r(14) ? Number(r(14)) : 0,
    CurrentEpisode: r(15) ? Number(r(15)) : 0,
    TotalSeasons:   r(16) ? Number(r(16)) : 0,
  };
}
...
function movieToRow(movie) {
  return [
    ...
    movie.AddedBy   ?? "",
    movie._type          ?? "",
    movie.CurrentSeason  ? String(movie.CurrentSeason)  : "",
    movie.CurrentEpisode ? String(movie.CurrentEpisode) : "",
    movie.TotalSeasons   ? String(movie.TotalSeasons)   : "",
  ];
}
...
export async function updateMovie(imdbID, changes) {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: sheetId(),
    range: `${WATCHLIST_SHEET}!A${sheetRow}:Q${sheetRow}`,
    valueInputOption: "RAW",
